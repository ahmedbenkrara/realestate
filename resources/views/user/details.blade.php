...
@section('title')
{{$listing->title.' | '.\App\Http\Controllers\SettingsController::settings()->title}}
@endsection
@section('content')
<style>
    #cust{
...
<section id="section-body">
                ...
                <div class="col-sm-12 col-xs-12">
                    <div class="header-detail table-list">
                        ...
                        <div class="header-right">
                            <ul class="actions">
                                @if(Auth::check() && Auth::user()->hasRole('user') && $test == null)
                                <li>
                                    <span><i class="fa fa-heart-o" id="addfav"></i></span>
                                </li>
                                @endif
                            </ul>
                            @if($listing->status == 1)
    ...
    <section class="section-detail-content">
            ...
            <div class="row">
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 container-contentbar">
                    <div class="detail-bar">
                        ...
                        <div class="detail-contact detail-block">
                            ...
                            <div class="detail-title-inner">
                                <h4 class="title-inner">{{__('login.lreview')}}</h4>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-6 col-xs-12">
                                        <div class="form-group">
                                            <input class="form-control" placeholder="{{__('login.name')}}" type="text" name="name" required>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-xs-12">
                                        <div class="form-group">
                                            <select class="form-control" name="rate" required>
                                                <option value="0" disabled>{{__('login.ratep')}}</option>
                                    ...
                                    <div class="col-sm-12 col-xs-12">
                                        <div class="form-group">
                                            <textarea class="form-control" rows="5" placeholder="{{__('login.comment')}}" name="description" required></textarea>
                                        </div>
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-secondary" style="background:#00aeef;">{{__('login.cmnt')}}</button>
                            </form>
                        </div>
                ...
                <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 col-md-offset-0 col-sm-offset-3 container-sidebar">
                    <aside id="sidebar">
                        ...
                        <div class="widget widget-download">
                            <div class="widget-top">
                                <h3 class="widget-title">{{__('login.delete')}}</h3>
                            </div>
                            <div class="widget-body">
                                <form action="/deletean" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$listing->id}}">
                                    <button type="submit" class="btn btn-danger" style="width:100%; background:red; font-weight:700; font-size:15px;">{{__('login.delete')}}</button>
                                </form>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="widget widget-recommend">
                            <div class="widget-top">
                                <h3 class="widget-title">{{__('login.rec')}}</h3>
                            </div>
                            <div class="widget-body">
                                ...
                                <div class="media">
                                    <div class="media-left">
                                        <figure class="item-thumb">
                                            <a class="hover-effect" href="{{url('/details/'.$sug->adresse->city.'/'.$sug->type->name.'/'.$sug->id)}}">
                                                <img alt="thumb" src="{{url('assets/images/posts')}}{{'/'.$sug->images[0]->path}}" style="width:100%; height:68px;">
                                            </a>
                                        </figure>
                                    </div>
                                    <div class="media-body">
                                        <h3 class="media-heading"><a href="{{url('/details/'.$sug->adresse->city.'/'.$sug->type->name.'/'.$sug->id)}}">{{$sug->title}}</a></h3>
                                        <div class="amenities">
                                            <p>{{$sug->detail->beds}} {{__('login.beds')}} • {{$sug->detail->baths}} {{__('login.baths')}} • {{$sug->detail->area_size}} sqft</p>
                                            <p>{{$sug->type->name}}</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="widget widget-categories">
                            <div class="widget-top">
                                <h3 class="widget-title">{{__('login.pcat')}}</h3>
                            </div>
                            <div class="widget-body">
                                <ul>
                                    @foreach($type as $t)
                                    <li><a target="_blank" href="{{url('/filter?title=&mins=&maxs=&minp=&maxp=&type=')}}{{$t->id}}">{{$t->name}}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="widget widget-reviews">
                            <div class="widget-top">
                                <h3 class="widget-title">{{__('login.lrev')}}</h3>
                            </div>
                            <div class="widget-body">
                                ...
                                <div class="media">
                                    <div class="media-left">
                                        <a href="#">
                                            <img class="media-object img-circle" src="{{url('/assets/images/posts')}}{{'/'.$listing->images[0]->path}}" alt="Thumb" style="width:50px; height:50px; border-radius:0;">
                                        </a>
                                    </div>
                                    <div class="media-body">
                                        <h3 class="media-heading"><a href="">{{$review->title}}</a></h3>
                                        <div class="rating">
                                            <span class="bottom-ratings">
                                                @for($i=0;$i<$review->rate;$i++)
                                                    <span class="fa-solid fa-star" style="font-family:'FontAwesome';"></span>
                                                @endfor

                                                @for($i=0;$i<(5-$review->rate);$i++)
                                                    <span class="fa fa-star-o" style="font-family:'FontAwesome';"></span>
                                                @endfor
                                            </span>
                                        </div>
                                        <p>
        ...
        <form action="/favorite" method="post" class="favform">
            @csrf
            <input type="hidden" value="{{$listing->id}}" name="annonce_id">
